...
if [ -d "$DIRROM" ]; then
    ...
    rm "$DIRROM"/.repo/manifests/crave.yaml* || true; # Removes existing crave.yamls
    curl -o "$DIRR/OM".repo/manifests/crave.yaml https://raw.githubusercontent.com/sounddrill31/crave_aosp_builder/main/configs/crave/crave.yaml.aosp # Downloads crave.yaml
    echo "--------------cloned crave yaml---------------"
    crave pull "$DIRROM"/out/target/product/a52q/*.zip
fi
